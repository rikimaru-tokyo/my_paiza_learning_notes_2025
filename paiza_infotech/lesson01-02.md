...
- [テクノロジー編01: コンピュータのデータ表現](#テクノロジー編01-コンピュータのデータ表現)
  ...
  - [06: 数値表現 - その2](#06-数値表現---その2)
    ...
    - [1の補数と2の補数](#1の補数と2の補数)
    - [m進数のとき](#m進数のとき)
    - [補数表現が用いられている理由](#補数表現が用いられている理由)
      - [PaizaAIで上記の計算結果でなぜ頭4bitが全部ゼロになるのか？を聞いてみた](#paizaaiで上記の計算結果でなぜ頭4bitが全部ゼロになるのかを聞いてみた)
    - [付録　補数・浮動小数点数についてわかりやすく](#付録補数浮動小数点数についてわかりやすく)
  - [07: 数値表現 - その3](#07-数値表現---その3)
    - [固定小数点数と浮動小数点数](#固定小数点数と浮動小数点数)
      - [PaizaAIに聞いてみた](#paizaaiに聞いてみた)
  - [08: 誤差](#08-誤差)
    - [真値とコンピュータ内部で表現できる値との差](#真値とコンピュータ内部で表現できる値との差)
    - [誤差の種類](#誤差の種類)
    - [オーバーフロー](#オーバーフロー)
    - [アンダーフロー](#アンダーフロー)
    - [丸め誤差](#丸め誤差)
      - [PaizaAIに質問　「循環小数0.11でありながら、なぜ指数部は123になるのでしょうか？エクセス数が含まれているためですか？」](#paizaaiに質問循環小数011でありながらなぜ指数部は123になるのでしょうかエクセス数が含まれているためですか)
      - [32ビット浮動小数の場合、有効数字は10進数で約6.92桁となる理由](#32ビット浮動小数の場合有効数字は10進数で約692桁となる理由)
    - [桁落ち](#桁落ち)
    - [情報落ち](#情報落ち)
    - [打切り誤差](#打切り誤差)
  - [09: シフト演算](#09-シフト演算)
    - [論理シフト](#論理シフト)
    - [算術シフト](#算術シフト)
    - [PaizaAIとの問答](#paizaaiとの問答)
- [テクノロジー編02: コンピュータで役に立つ数学について学んでいこう](#テクノロジー編02-コンピュータで役に立つ数学について学んでいこう)
  - [01: コンピュータで役に立つ数学](#01-コンピュータで役に立つ数学)
    - [論理演算](#論理演算)
    - [2進数の計算](#2進数の計算)
    - [確率と統計](#確率と統計)
    - [グラフ理論](#グラフ理論)
    - [待ち行列理論](#待ち行列理論)
    - [数値計算、数式処理、最適化問題](#数値計算数式処理最適化問題)
      - [数値計算](#数値計算)
      - [数式処理](#数式処理)
      - [最適化問題](#最適化問題)
  - [02: 論理演算（AND, OR, NOT）](#02-論理演算and-or-not)
    - [演算](#演算)
    - [論理演算](#論理演算-1)
    - [論理積演算(AND演算)](#論理積演算and演算)
    - [論理和演算(OR演算)](#論理和演算or演算)
    - [論理否定演算(NOT演算)](#論理否定演算not演算)
  - [03: 論理演算（NAND、NOR、XOR）](#03-論理演算nandnorxor)
    - [否定論理積演算(NAND演算)](#否定論理積演算nand演算)
    - [否定論理和演算(NOR演算)](#否定論理和演算nor演算)
    - [排他的論理和演算(XOR演算または EOR演算)](#排他的論理和演算xor演算または-eor演算)
  - [04: ド・モルガンの法則](#04-ドモルガンの法則)
  - [05: 確率](#05-確率)
    - [確率とは](#確率とは)
  - [06: 統計](#06-統計)
    - [統計とは](#統計とは)
  ...
  - [08: 待ち行列理論](#08-待ち行列理論)
    ...
    - [平均待ち時間](#平均待ち時間)
    - [平均応答時間](#平均応答時間)
  - [09: 数値計算、数式処理、最適化問題](#09-数値計算数式処理最適化問題)
    - [数値計算](#数値計算-1)
      - [連立一次方程式](#連立一次方程式)
      - [連立方程式の解法](#連立方程式の解法)
    - [数式処理](#数式処理-1)
